...
export const getAllDataset = async (setDataset: React.Dispatch<React.SetStateAction<DatasetResponse[]>>, 
    ...
    try {
        const response = await axios.get("https://outfit-recs-421406.et.r.appspot.com/api/v1/dataset/")
...
export const getAllImage = async (datasedId: number, setImageData: React.Dispatch<React.SetStateAction<ImageResponse[]>>) => {
    try {
        const response = await axios.get(`https://outfit-recs-421406.et.r.appspot.com/api/v1/clothes/${datasedId}/`)
        const data = response.data.data
        if(data){
            setImageData(data)
        }
...
export const getImageDetail = async (imageId: number | null, setImageDetail: React.Dispatch<React.SetStateAction<ImageResponse | undefined>>) => {
    try {
        const response = await axios.get(`https://outfit-recs-421406.et.r.appspot.com/api/v1/clothes/detail/${imageId}/`)
        const data = response.data.data
        if(data){
            setImageDetail(data)
        }
...
export const getSegmentation = async (imageId: number | null, 
    ...
    try {
        const response = await axios.get(`https://outfit-recs-421406.et.r.appspot.com/api/v1/clothes/segmentation/${imageId}/`)
        const data = response.data.data
        if(data){
